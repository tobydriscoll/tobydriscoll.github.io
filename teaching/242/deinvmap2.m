...
% ODE
  ...
  odetol = max(tol,1e-5*(newton));

  % Rescale dependent coordinate
  scale = (wp(~done) - w0(:));

  % Solve ODE
  z0 = [real(z0(1));imag(z0(1))];
  [t,y] = ode23('deimapfun',[0,0.5,1],z0,odeset('abstol',odetol),...
      scale(1),z,beta,c);
  idx = find(~done);
  zp(idx(1)) = y(end,1)+sqrt(-1)*y(end,2);
  out = abs(zp) > 1;
  zp(out) = sign(zp(out));
...
% Newton iterations
  ...
  wp = wp(:);
  for j = find(~done(:).')
    if j>1, zn(j) = zn(j-1); end
    k = 0;
    while ~done(j) && k < maxiter
      F = wp(j) - demap(zn(j),w,beta,z,c,qdat);
      dF = c*zn(j).^(-2) * exp( sum(beta.* log(1-zn(j)./z)) );
      zn(j) = zn(j) + F/dF;
      done(j) = (abs(F)< tol);
      k = k+1;
    end
